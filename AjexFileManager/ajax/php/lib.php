...
global $cfg;
if (!is_dir($cfg['root'])) {
	mkdirs($cfg['root'], $cfg['chmod']['folder']);
}
$type = array($cfg['thumb']['dir'], 'file', 'flash', 'image');
	for ($i=-1;++$i<4;) {
		$d = $cfg['root'] . $type[$i];
		if (!is_dir($d)) {
			mkdirs($d, $cfg['chmod']['folder']);
		}
	}
...
function getFreeFileName($fileName, $dir)
{
	...
	$format = strtolower(substr($fileName, $dotPos+1, ($strlen-$dotPos)));
...
function isWork()
{
	...
	if (!is_dir($cfg['root'])) {
		...
		if (mkdirs($cfg['root'], $cfg['chmod']['folder'])) {
			...
			echo 'Failed created, You need to create the folder manually, or set the right<br />Р’Р°Рј РЅРµРѕР±С…РѕРґРёРјРѕ СЃРѕР·РґР°С‚СЊ РїР°РїРєСѓ РІСЂСѓС‡РЅСѓСЋ, РёР»Рё РІС‹СЃС‚Р°РІРёС‚СЊ РїСЂР°РІР°';
			exit;
		}
	} elseif (!is_writable($cfg['root'])) {
		echo 'No write access to folder: ' . $cfg['root'], '<br />РќРµС‚ РґРѕСЃС‚СѓРїР° РЅР° Р·Р°РїРёСЃСЊ.';
	} else {
		echo '<font color=green>Root directory in order<br />РљРѕСЂРЅРµРІР°СЏ РґРёСЂРµРєС‚РѕСЂРёСЏ РІ РїРѕСЂСЏРґРєРµ.</font>';
	}
	echo '<hr />';
	...
	for ($i=-1;++$i<4;) {
		...
		if (!is_dir($d)) {
			...
			echo 'No write access to folder: ' . $d, '<br />РќРµС‚ РґРѕСЃС‚СѓРїР° РЅР° Р·Р°РїРёСЃСЊ.';
		} else {
