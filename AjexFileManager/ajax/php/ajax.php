...
if(!$ckeditor->can_upload())
	exit();
	
define( 'DS', DIRECTORY_SEPARATOR );
$rootFolder = explode(DS,dirname(__FILE__));
array_splice($rootFolder,-5);
$doc_root = explode(DIR_SEP,$_SERVER['DOCUMENT_ROOT']);
$eve = '';
foreach ($rootFolder as $id=>$item)
{
	if(!isset($doc_root[$id]) || $doc_root[$id] != $rootFolder[$id])
		$eve .= $rootFolder[$id].DIR_SEP;
}
